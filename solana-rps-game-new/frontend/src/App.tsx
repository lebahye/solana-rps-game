...
import AdminDashboardView from './views/AdminDashboardView'; // Import for AdminDashboardView
import { ThemeProvider } from './context/ThemeContext'; // Import ThemeProvider
import ThemeToggle from './components/ThemeToggle'; // Import ThemeToggle
import ErrorNotification from './components/ErrorNotification'; // Import ErrorNotification
import './styles/themes.css'; // Import theme styles
import { errorHandlerService, ErrorDetails } from './services/error-handler-service'; // Import error handler
import { analyticsService, AnalyticsEventType } from './services/analytics-service'; // Import analytics service
...
const RPSGame: React.FC = () => {
  ...
  const [errorNotification, setErrorNotification] = useState<ErrorDetails | null>(null);
  ...
  useEffect(() => {
    if (connected && publicKey) {
      analyticsService.setUserId(publicKey.toString());
      analyticsService.trackWalletConnection('Solana');
    } else {
    ...
    const initClient = async () => {
      if (connected && publicKey && signTransaction) {
        try {
          ...
          const errorDetails = errorHandlerService.handleTransactionError(error, {
            walletAddress: publicKey,
            operation: 'initialize_client'
          });

          setErrorNotification(errorDetails);
        }
      } else {
  ...
  return (
    <div className="app-container">
      ...
      <nav className={`app-nav ${isMobileMenuOpen ? 'open' : ''}`}>
        ...
        <button onClick={() => setCurrentView(GameView.PROFILE)}>Profile</button>
        <button onClick={() => setCurrentView(GameView.ADMIN)}>Admin Dashboard</button> {/* Fixed Admin Dashboard button */}
      </nav>
